...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login(email, password, 'mock-captcha-token');
  ...
  const handleOAuthLogin = (provider: 'google' | 'yahoo' | 'linkedin') => {
    // Store intent to return to discover after OAuth
    localStorage.setItem('oauth_return_path', '/discover');
    // For OAuth, we'll assume investor role by default - could add role selection
    window.location.href = api.getOAuthUrl(provider, 'investor');
  };
